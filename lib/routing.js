...
// ============================================================
export async function generateRouteORS(apiKey, start, distanceM, type, seed = 0) {
  const url = `${ORS_BASE}/v2/directions/foot-walking/geojson`;

  let body;

  if (type === 'loop') {
    // Mode round_trip : ORS génère une vraie boucle fermée
    body = {
      coordinates: [[start.lng, start.lat]],
      options: {
        round_trip: {
          length: distanceM,          // distance cible en mètres
          points: 5,                  // waypoints intermédiaires
          seed: seed,                 // graine pour varier les routes
        },
      },
    ...
    // Aller-retour : point milieu calculé, retour au départ
    const midpoint = computeLoopMidpoint(start.lat, start.lng, distanceM / 2000);
    body = {
      coordinates: [
        [start.lng, start.lat],
